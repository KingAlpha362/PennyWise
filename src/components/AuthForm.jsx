...
import { Eye, EyeOff, Mail, KeyRound, Sparkles as SparklesIcon } from "lucide-react";
...
export default function AuthForm({ initialMode = 'signin', onAuthSuccess, onBack }) {
  ...
  return (
    <div className="min-h-screen bg-bg flex font-sans text-text transition-colors">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-card border-r border-border p-12 flex-col justify-between relative overflow-hidden">
        ...
        <button 
          onClick={onBack} 
          className="flex items-center gap-2 text-sm text-text-muted hover:text-text transition-all z-10 font-bold cursor-pointer"
        >
          <ArrowLeft size={16} /> Back to main site
        </button>

        <div className="relative z-10 max-w-md">
          <span className="text-2xl font-extrabold text-[#16A34A]">Penny<span className="text-[#c9922a]">Wise</span></span>
          <h2 className="text-4xl font-extrabold tracking-tight mt-6 leading-tight text-text">
            {mode === 'signin' 
          ...
          <div className="mt-10 space-y-4">
            <div className="flex items-center gap-3 bg-bg border border-border p-4 rounded-2xl shadow-sm">
              <CheckCircle className="text-[#16A34A] flex-shrink-0" size={20} />
              <p className="text-xs text-text font-bold">Live cross-account balance mapping</p>
            </div>
            <div className="flex items-center gap-3 bg-bg border border-border p-4 rounded-2xl shadow-sm">
              <Shield className="text-[#16A34A] flex-shrink-0" size={20} />
              <p className="text-xs text-text font-bold">AES-256 hardware level encryption schemas</p>
            </div>
            <div className="flex items-center gap-3 bg-bg border border-border p-4 rounded-2xl shadow-sm">
              <Sparkles className="text-[#c9922a] flex-shrink-0" size={20} />
              <p className="text-xs text-text font-bold">Real-time LLM predictive allocation routines</p>
            </div>
          </div>
        </div>

        <p className="text-xs text-text-muted z-10 font-bold">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 md:p-16 relative">
        <button 
          onClick={onBack} 
          className="lg:hidden absolute top-6 left-6 flex items-center gap-2 text-xs text-text-muted hover:text-text transition-all font-bold cursor-pointer"
        >
